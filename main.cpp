...
user cstr2struct(const char *tmp_str) {
...
void struct2json(std::vector<user> &users, std::ofstream &ofs) {
    ...
    for (size_t i = 0; i < arg.size(); ++i) {
        sort(users.begin(), users.end(), [=](const user &left, const user &right) {
            if (arg[i] == "name") {
...
//  ./letuchka1 "Sample User:13" "Aummy Prepod:29" "Dummy Listener:20" "Fmart Listener:14"
